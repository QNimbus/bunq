...
#   timestamp: 2023-12-05T15:13:48+00:00
...
class ActionType(Enum):
    DUMMY = 'DUMMY'
    REQUEST_FROM_EXPENSE = 'REQUEST_FROM_EXPENSE'
    TRANSFER_REMAINING_BALANCE = 'TRANSFER_REMAINING_BALANCE'
    TRANSFER_INCOMING_PAYMENT = 'TRANSFER_INCOMING_PAYMENT'
...
class RequestFromExpenseActionData(BaseModel):
    model_config = ConfigDict(
        extra='forbid',
    )
...
class TransferIncomingPaymentActionData(BaseModel):
    model_config = ConfigDict(
        extra='forbid',
    )
    only_allow_own_accounts: Optional[bool] = True
    transfer_payment_to: BankAccount
    description: Optional[str] = None


class TransferRemainingBalanceActionData(BaseModel):
    model_config = ConfigDict(
        extra='forbid',
    )
    only_allow_own_accounts: Optional[bool] = True
    transfer_remaining_balance_to: BankAccount
    description: Optional[str] = None
...
class RuleModel(RootModel[List[RuleRoot]]):
    root: List[RuleRoot]
